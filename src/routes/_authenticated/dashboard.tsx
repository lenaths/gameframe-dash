        ...
        {isLoading ? (
          ...
          <div className="grid gap-3">
            {(data?.servers ?? []).map((s) => (
              <div
                key={s.id}
                className="rounded-xl border border-border/60 bg-surface p-5 flex flex-wrap items-center gap-4 justify-between"
              >
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <div className="text-sm text-muted-foreground">
                      {s.plans?.game} · {s.plans?.name} ·{" "}
                      {((s.plans?.ram_mb ?? 0) / 1024).toFixed(0)} GB RAM
                    </div>
                    {s.error_message && (
                      <div className="text-xs text-destructive mt-1">{s.error_message}</div>
                    )}
                  </div>
                </div>
                <div className="flex items-center gap-3">
                  <span
                    className={`text-xs uppercase tracking-wider px-2.5 py-1 rounded-full border ${statusColor[s.status] ?? ""}`}
                  >
                    {s.status}
                  </span>
                  {s.status === "active" && (
                    <div className="flex gap-1.5 items-center">
                      <Button
                        ...
                        onClick={() => power.mutate({ orderId: s.id, signal: "start" })}
                      >
                      ...
                      <Button
                        ...
                        variant="outline"
                        disabled={power.isPending}
                        onClick={() => power.mutate({ orderId: s.id, signal: "restart" })}
                      >
                      ...
                      <Button
                        ...
                        variant="outline"
                        disabled={power.isPending}
                        onClick={() => power.mutate({ orderId: s.id, signal: "stop" })}
                      >
